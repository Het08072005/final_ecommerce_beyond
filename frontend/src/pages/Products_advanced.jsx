...
const Products = () => {
  ...
  const genders = useMemo(() => {
    const unique = [...new Set(allProducts.map(p => p.gender).filter(Boolean))];
    return unique.sort();
  }, [allProducts]);
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const response = await axios.get('/smart-search', { params: { q: query } });
      setSearchResults(response.data || { products: [], appliedFilters: {} });
    } catch (error) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedGenders.length > 0) {
      filtered = filtered.filter(p =>
        selectedGenders.some(g => p.gender?.toLowerCase() === g.toLowerCase())
      );
    }
    ...
    return filtered;
  }, [searchResults, allProducts, selectedGenders, selectedColors, selectedSizes, selectedOccasions, priceRange, sortBy]);
  ...
  const activeFiltersCount = selectedGenders.length + selectedColors.length + selectedSizes.length + selectedOccasions.length;
            ...
            <div className="filter-tags">
              {detectedFilters.priceRange && (
                <span className="filter-tag price-tag">
                  ₹{detectedFilters.priceRange[0].toLocaleString()} - ₹{detectedFilters.priceRange[1].toLocaleString()}
                </span>
              )}
              {detectedFilters.gender?.map(g => (
                <span key={g} className="filter-tag gender-tag">{g}</span>
              ))}
              {detectedFilters.colors?.map(c => (
                <span key={c} className="filter-tag color-tag">{c}</span>
              ))}
              {detectedFilters.sizes?.map(s => (
                <span key={s} className="filter-tag size-tag">Size {s}</span>
              ))}
              {detectedFilters.occasions?.map(o => (
                <span key={o} className="filter-tag occasion-tag">{o}</span>
              ))}
              {detectedFilters.keywords?.map(k => (
                <span key={k} className="filter-tag keyword-tag">{k}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="products-layout">
        ...
        <aside className={`filters-sidebar ${showFilters ? 'open' : 'closed'}`}>
          ...
          <div className="filter-section">
            ...
            >
              <span>Gender</span>
              <span className="toggle-icon">{expandedSections.gender ? '−' : '+'}</span>
            </button>
          ...
          <div className="filter-section">
            ...
            >
              <span>Occasions</span>
              <span className="toggle-icon">{expandedSections.occasions ? '−' : '+'}</span>
            </button>
          ...
          <div className="products-toolbar">
            <div className="results-info">
              <p>
                ...
                {activeFiltersCount > 0 && ` with ${activeFiltersCount} filter(s)`}
              </p>
            </div>
            <select
              value={sortBy}
              ...
              {filteredProducts.map(product => (
                <div key={product.id} className="product-card">
                  ...
                  <div className="product-info">
                    ...
                    {product.gender && <p className="gender">{product.gender}</p>}
